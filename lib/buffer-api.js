var util = require('util'),
  ...
  bufferCore = {
    ...
    createRequest: function (method, endpoint, options, responseFilter) {
      ...
      return new Promise(function (resolve, reject) {
        self.runRequest(requestOptions, function (err, res, body) {
          ...
          if (err) {
            return reject(err)
          } else if (res.statusCode !== 200 || (body && typeof(body.success) !== "undefined" && !body.success)) {
            if (body) {
              if (body.error) {
                err = new Error(util.format(body.error))
              } else {
                err = new Error(util.format(body.message))
              }
    ...
    buildRequest: function (method, endpoint, options) {
      ...
      return _.merge(opts, _.omit(options, ['authorize']))
    },
...
module.exports = function bufferApi(accessToken) {
  var client = Object.create(_.assign(
    {},
    bufferCore,
    bufferApiObjects
